...
| 罕见     | 0x006E3D17               | 角色皮肤相关。常见于玩家在载具内/进出载具时更换皮肤             | 确保玩家处于步行状态时更换皮肤                                                |
| 罕见     | 0x0058370A               | 载具/视角相关。当脚本尝试将玩家传送至未完成加载的载具时触发     | 创建新载具后等待数百毫秒再传送玩家，或使用 SetCameraBehindPlayer 预先调整视角 |
...
| 常见     | 0x007F0BF7               | 载具改装异常（如为摩托车添加氮气加速/尾翼）或客户端载具模组冲突 | 使用论坛提供的改装校验脚本                                                    |
...
| 错误信息                         | 原因                                                                                         | 解决方案                          |
| -------------------------------- | -------------------------------------------------------------------------------------------- | --------------------------------- |
| Exception 0xC0000005 at 0x5E5815 | 复杂系统级错误（涉及角色动画混合/音频处理）。常见于进入载具时触发脚本事件（如获得武器/传送） | -                                 |
...
| Exception 0xC0000005 at 0x7F120E | 非法载具改装（客户端安装的 handling.cfg/vehicles.ide 修改未被 SA:MP 加载）                   | 移除载具数据修改以保证公平性      |
